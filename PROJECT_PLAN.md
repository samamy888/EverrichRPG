...
- 所有新資產從 `docs/ART_PIPELINE.md` 開始，並以 `docs/PIXEL_ART_STYLE_GUIDE.md` 與 `docs/CHARACTER_ASSET_SPEC.md` 作為正式美術規範。
- 之後產生資產時，需先確認視角、比例、色彩、命名與動畫規則是否符合該規範。
- 既有資產若與規範落差太大，優先安排重畫或替換，避免同一張地圖混用多種風格。
