...
// 注意：此檔案是從 ConcourseScene 拆分出的 A 區場景版本，
// 目前邏輯與版面與原始 ConcourseScene 相同（後續可再依 A 區專屬需求調整）。
type EnterData = { spawnX?: number; spawnY?: number };

export class ConcourseAScene extends Phaser.Scene {
  ...
  constructor() { super('ConcourseAScene'); }
  ...
  create() {
    ...
    for (const e of entries.filter(e => e.side === 'top')) {
    ...
    // world/camera bounds and player
    ...
    // Spawn crowd in central corridor (reuse common helper)
  ...
  update() {
    ...
    // Door interaction (open store)
    // 已移除邊界觸發的場景切換（A -> Hall）

    let nearest: { world: Phaser.Math.Vector2; id: string; label: string } | null = null; let nd = 1e9;
    for (const d of this.doors) {
      const dx = d.world.x - this.player.x; const dy = d.world.y - this.player.y; const dd = Math.hypot(dx, dy);
