...
// B 區場景：先行複製自原 ConcourseScene，後續可縮減至只保留 B 區元素。
type EnterData = { spawnX?: number; spawnY?: number };

export class ConcourseBScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (const e of entries.filter(e => e.side === 'bottom')) {
  ...
  update() {
    ...
    // 固定為 B 區
    this.registry.set('location', 'B 區');
    this.registry.set('locationType', 'concourse-B');
