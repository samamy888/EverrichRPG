...
import { T2_FACILITIES_V3_CENTRAL } from '../data/facilitiesT2V3';
import {
...
export class TPE2CentralHallV3Scene extends BaseScene {
  ...
  constructor() { super('TPE2CentralHallV3Scene'); }
  ...
  preload() {
    ...
    const uniqueProps = Array.from(new Set([
      ...T2_FACILITIES_V3_CENTRAL.map(f => this.floorplanPropKey(f.texture.replace('prop-', ''))),
      ...TPE2_FLOORPLAN_PROP_KEYS,
  ...
  create(data: BaseSceneData) {
    ...
    T2_FACILITIES_V3_CENTRAL.forEach(fac => this.addFacility(fac));
    ...
    if (this.mapshotMode || this.tilecapMode) {
      ...
      this.spawnCrowd();
      attachOthers(this, { getArea: () => 't2_v3_central', crossArea: false });
    }

    this.setLocation('T2 重製 中央大廳', 'concourse');
  ...
  private resolveZone(x: number, y: number) {
    const hits = TPE2CentralHallV3Scene.ZONES.filter((z) => (
  ...
  private handleAreaTransitions() {
    ...
    if (this.player.y <= 88) {
      ...
      this.changeScene('TPE2NorthDZoneV3Scene', {
        spawnX: Phaser.Math.Clamp(this.player.x, 180, this.worldW - 180),
        spawnY: this.worldH - 220,
      });
      return true;
    }
    if (this.player.y >= this.worldH - 88) {
      this.areaTransitionLock = true;
      this.changeScene('TPE2SouthCZoneV3Scene', {
        spawnX: Phaser.Math.Clamp(this.player.x, 180, this.worldW - 180),
        spawnY: 220,
      });
      return true;
  ...
  update() {
    ...
    this.updateNetworkMovement('t2_v3_central');
