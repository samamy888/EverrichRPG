...
import { T2_FACILITIES_V3_NORTH_D } from '../data/facilitiesT2V3';
import {
...
export class TPE2NorthDZoneV3Scene extends BaseScene {
  ...
  constructor() { super('TPE2NorthDZoneV3Scene'); }
  ...
  preload() {
    ...
    const uniqueProps = Array.from(new Set([
      ...T2_FACILITIES_V3_NORTH_D.map(f => this.floorplanPropKey(f.texture.replace('prop-', ''))),
      ...TPE2_FLOORPLAN_PROP_KEYS,
  ...
  create(data: BaseSceneData) {
    ...
    T2_FACILITIES_V3_NORTH_D.forEach(fac => this.addFacility(fac));
    ...
    if (this.mapshotMode || this.tilecapMode) {
      ...
      this.spawnCrowd();
      attachOthers(this, { getArea: () => 't2_v3_north_d', crossArea: false });
    }

    this.setLocation('T2 重製 D 區', 'concourse');
  ...
  private resolveZone(x: number, y: number) {
    const hits = TPE2NorthDZoneV3Scene.ZONES.filter((z) => (
  ...
  update() {
    ...
    this.updateNetworkMovement('t2_v3_north_d');
