...
export class TPE2LobbyV2Scene extends BaseScene {
  ...
  constructor() { super('TPE2LobbyV2Scene'); }
  ...
  private openStore(storeId: string) {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
      ...
      this.scene.pause();
      this.scene.launch('StoreScene', { storeId, returnTo: 'TPE2LobbyV2Scene' });
    });
  }
  ...
  private resolveZone(x: number, y: number) {
    const hits = TPE2LobbyV2Scene.ZONES.filter((z) => (
